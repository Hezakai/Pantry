//This code sets up a one-to-many relationship between the User and Inventory models using Sequelize associations. It allows you to associate inventories with users and ensures that when a user is deleted, their associated inventories are also deleted (cascading delete). Conversely, each Inventory is associated with a single user through a foreign key relationship.
...
//Establishes a one-to-many association between the User model and the Project model. This means that a single user can be associated with multiple projects.
